...
export function createApp() {
    ...
    app.post("/webhook/receive-msg", async (req, res) => {
        try {
            ...
            const phone = pick(incomingData.customerNumber, incomingData.phone, incomingData.phoneNumber, incomingData.phone_number, incomingData.from, incomingData.sender, parsedContent.phone, parsedContent.customerNumber);
            const childName = pick(incomingData.childName, incomingData.child_name, incomingData.name, incomingData.patientName, incomingData.patient_name, parsedContent.childName, parsedContent.name, incomingData.userDetails?.name);
            const parentName = pick(incomingData.parentName, incomingData.parent_name, incomingData.parent, incomingData.guardianName, parsedContent.parentName, parsedContent.parent, incomingData.userDetails?.parentName);
            const age = pick(incomingData.age, incomingData.child_age, incomingData.patientAge, parsedContent.age, incomingData.userDetails?.age);
    ...
    app.patch("/webhook/messages/:id", async (req, res) => {
        try {
            const { id } = req.params;
            const { status, bookingStatus, appointmentDate, appointmentTime, assignedTherapist, assignedTherapistId, concern, childName, parentName, age, paymentStatus, paymentMode, transactionId, } = req.body;
            const updateData = {};
            if (status !== undefined)
            ...
            if (assignedTherapist !== undefined)
                updateData.assignedTherapist = assignedTherapist;
            if (assignedTherapistId !== undefined)
                updateData.assignedTherapistId = assignedTherapistId;
            ...
            if (updated.phone) {
                try {
                    ...
                    if (db) {
                        ...
                        await db.collection("chatbotsubmissions").updateMany({ phone: { $in: phoneQueries } }, {
                            $set: {
                        ...
                        const appointmentUpdate = { updatedAt: new Date().toISOString() };
                        if (paymentStatus)
                            appointmentUpdate.paymentStatus = paymentStatus;
                        if (status)
                            appointmentUpdate.bookingStatus = status;
                        if (bookingStatus)
                            appointmentUpdate.bookingStatus = bookingStatus;
                        if (appointmentDate)
                            appointmentUpdate.appointmentDate = appointmentDate;
                        if (appointmentTime)
                            appointmentUpdate.appointmentTime = appointmentTime;
                        if (assignedTherapist)
                            appointmentUpdate.therapistName = assignedTherapist;
                        if (transactionId)
                            appointmentUpdate.transactionId = transactionId;
                        await db.collection("appointments").updateMany({ phoneNumber: { $in: phoneQueries } }, { $set: appointmentUpdate });
                    }
                }
                catch (syncErr) {
