...
function extractPaymentData(payload) {
    ...
    const rawPhone = pickFirst(data, "phoneNumber", "phone_number", "phone", "mobile", "mobileNumber", "customerPhone", "customer_phone", "sender", "from", "wa_id", "wa_number", "whatsapp_number", "customerNumber") ||
    ...
    const transactionId = pickFirst(data, "transaction_id", "transactionId", "txn_id", "txnId", "payment_id", "paymentId", "razorpay_payment_id", "gateway_payment_id", "reference_id", "ref_id", "requestId", "uuid") ||
        ...
        "";
    // Extract order ID
    const orderId = pickFirst(data, "order_id", "orderId", "booking_id", "bookingId", "order_reference_id") ||
    ...
    const event = pickFirst(body, "event", "eventType", "type", "action") ||
    ...
    const amount = pickFirst(data, "amount", "order_amount", "paid_amount", "total_amount") ||
 ...
 */
export async function handleMsg91PaymentWebhook(req, res) {
    ...
    try {
        ...
        const isPaymentSuccess = !event || // If generic webhook payload without event, treat received confirmation as success
        ...
        if (appointment) {
            const updateResult = await appointmentsCollection.updateOne({ _id: appointment._id }, {
                $set: {
