...
async function triggerConfirmationEmailIfPaid(record) {
    ...
    if (!recipientEmail || record.emailSent)
        return;
...
async function triggerPostPaymentActions(record) {
    if (!record)
        return;
    if (!record.emailSent) {
        try {
...
async function createGatewayQrCode(amountPaise, orderNumber, localOrderId) {
    ...
    console.log("=====================================");
    if (!response.ok) {
        const description = payload?.error?.description || payload?.message || "Unable to create Razorpay QR code";
...
paymentsRouter.post("/razorpay/verify", async (req, res, next) => {
    try {
        ...
        triggerPostPaymentActions(updatedOrder).catch((err) => console.error("🚨 Error in post-payment actions:", err));
        res.json({
            success: true,
...
async function createPaymentLinkHandler(req, res, next) {
    try {
        ...
        const isShopPurchase = category.toLowerCase() === "shop" ||
...
async function verifyLinkStatusHandler(req, res, next) {
    try {
        requireRazorpayConfig();
        ...
        if (isPaid) {
            ...
            if (isMongoConnected()) {
                ...
                if (record) {
                    triggerPostPaymentActions(record).catch((err) => console.error("🚨 Error in post-payment actions:", err));
                }
            ...
            if (entity) {
                ...
                if (isMongoConnected()) {
                    ...
                    const record = (await getMongoDb().collection("orders").findOne(query)) ||
                        (await getMongoDb().collection("donations").findOne(query));
                    if (record) {
                        triggerPostPaymentActions(record).catch((err) => console.error("🚨 Error in post-payment actions from webhook:", err));
                    }
                }
...
async function createQrHandler(req, res, next) {
    try {
        requireRazorpayConfig();
        const userId = getAuthenticatedUserId(req);
        const body = req.body ?? {};
        ...
        res.status(201).json({
            ...
            qrCode: {
                ...
                isFallback: Boolean(qrCode.isFallback),
            },
        });
    }
    catch (error) {
...
async function qrStatusHandler(req, res, next) {
    try {
        requireRazorpayConfig();
            ...
            if (response.ok && payload.items && payload.items.length > 0) {
                ...
                if (payment.status === "captured" || payment.status === "authorized") {
                    ...
                    if (isMongoConnected()) {
                        ...
                        const record = await getMongoDb().collection("orders").findOne(query);
                        if (record) {
                            triggerPostPaymentActions(record).catch((err) => console.error("🚨 Error in post-payment actions from QR poll:", err));
                        }
                    }
                    res.json({
                        success: true,
