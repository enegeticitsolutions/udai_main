...
userRouter.get("/addresses", async (req, res, next) => {
    try {
        const data = await getUserAddresses(req.user.id);
        res.json({ success: true, data });
    }
    catch (error) {
        next(error);
...
userRouter.get("/cart", async (req, res, next) => {
    try {
        const data = await getUserCart(req.user.id);
        res.json({ success: true, data });
    }
    catch (error) {
        next(error);
...
userRouter.put("/cart", async (req, res, next) => {
    try {
        ...
        const data = await saveUserCart(req.user.id, payload.items);
...
userRouter.post("/cart/merge", async (req, res, next) => {
    try {
        const payload = cartSchema.parse(req.body);
        const data = await mergeUserCart(req.user.id, payload.items);
...
userRouter.post("/address", async (req, res, next) => {
    try {
        ...
        const data = await createUserAddress(req.user.id, payload);
