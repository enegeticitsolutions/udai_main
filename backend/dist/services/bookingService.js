...
export async function loadTherapists(department) {
    ...
    const deptFilter = normalizedDept === "Child and Parental Counselling"
...
export async function assignTherapist(department, date, time) {
    ...
    let db = isMongoConnected() ? getMongoDb() : mongoose.connection.db;
    if (!db) {
        await connectMongoDb();
        db = isMongoConnected() ? getMongoDb() : mongoose.connection.db;
    }
    const scoredTherapists = await Promise.all(eligibleTherapists.map(async (t) => {
        const tName = t.name;
