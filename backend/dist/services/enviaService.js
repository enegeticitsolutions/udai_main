 ...
 */
export function buildEnviaShipmentPayload(order) {
    const shippingAddress = order.shippingAddress || {};
    const customerName = (order.customerName ||
        shippingAddress.fullName ||
        shippingAddress.name ||
        "Customer").trim();
    const customerPhone = normalizePhone(order.customerPhone || shippingAddress.mobile || shippingAddress.phone);
    const customerEmail = (order.customerEmail ||
        shippingAddress.email ||
        order.email ||
        "airishmultibrand487@gmail.com").trim();
    const street = [
        ...
        .filter(Boolean)
        .join(", ") || "Main Street";
    const district = (shippingAddress.area ||
    ...
    const itemCount = items.reduce((sum, it) => sum + (Number(it.quantity) || 1), 0) || 1;
 ...
 */
export async function createEnviaShipment(order) {
    ...
    try {
        ...
        const trackingNumber = itemData?.trackingNumber ||
            ...
            body?.trackingNumber ||
            "";
        const shippingLabelUrl = itemData?.label ||
            itemData?.labelUrl ||
            itemData?.label_url ||
            body?.label ||
            "";
        const enviaShipmentId = itemData?.shipmentId ||
            itemData?.shipment_id ||
            itemData?.id ||
            body?.shipmentId ||
            "";
        const carrier = itemData?.carrier ||
 ...
 */
export function isShopPhysicalOrder(record) {
    if (!record)
        return false;
    const isDonationCategory = record.category === "meal" ||
 ...
 */
export async function processEnviaShipmentForOrder(orderRecord) {
    if (!orderRecord)
        return null;
