...
export function getSupabaseClient() {
  if (!supabaseClient) {
    ...
    supabaseClient = createClient(
      config.supabaseUrl,
      config.supabaseServiceRole || config.supabaseAnon
    );
  }
  return supabaseClient;
}

export async function uploadToSupabase(filePath: string, originalName: string, mimeType: string): Promise<string> {
