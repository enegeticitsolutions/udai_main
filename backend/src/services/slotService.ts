...
function getStoredSlotsPath(): string {
  return path.join(config.storageDir, "therapist_slots.json");
}

function getNext5Dates(): string[] {
  const dates: string[] = [];
  const now = new Date();
  for (let i = 1; i <= 5; i++) {
...
export async function seedTherapistSlots(): Promise<{ seededCount: number; dates: string[] }> {
  await connectMongoDb();
  const next5Dates = getNext5Dates();

  let therapists: Array<{ id: string; name: string }> = [];
  ...
  if (isMongoConnected()) {
    try {
      await Promise.race([
        Promise.all(
          newSlots.map((slot) =>
            TherapistSlotModel.updateOne(
              { therapistId: slot.therapistId, date: slot.date, time: slot.time },
              { $setOnInsert: slot },
              { upsert: true }
            )
          )
...
export async function getAvailableSlotsForDate(
  ...
  let slotsFromDb: ITherapistSlot[] = [];

  if (isMongoConnected()) {
    const query: Record<string, unknown> = { date: targetDate, status: "available" };
    if (therapistId) {
  ...
  if (slotsFromDb.length === 0) {
    ...
    try {
      ...
      const jsonSlots: ITherapistSlot[] = JSON.parse(content);
      slotsFromDb = jsonSlots.filter(
        (s) => s.date === targetDate && (therapistId ? s.therapistId === therapistId : true)
      );
    } catch {
  ...
  if (isMongoConnected()) {
    ...
    for (const b of bookedWebhooks) {
      if (b.appointmentTime) bookedTimes.add(b.appointmentTime);
    }
  }
