...
export function normalizeSignupPhone(input: Pick<SignupInput, "phone" | "mobileNumber">) {
  return (input.phone ?? input.mobileNumber)?.trim() || undefined;
}

export async function findExistingSignupUser(input: Pick<SignupInput, "email" | "phone" | "mobileNumber">) {
  const email = input.email.trim().toLowerCase();
  const phone = normalizeSignupPhone(input);
  const duplicateChecks: Record<string, string>[] = [{ email }];

  if (phone) {
    duplicateChecks.push({ phone }, { mobileNumber: phone });
  }

  return getMongoDb().collection<UserDocument>("users").findOne({ $or: duplicateChecks });
}
...
function otpEmailTemplate(title: string, otp: string) {
  return `
...
export async function requestSignupVerification(input: SignupInput) {
  ...
  const pendingSignup: Omit<PendingSignupDocument, "_id"> = {
    name: input.name.trim(),
    email,
    ...(phone ? { phone } : {}),
    passwordHash,
...
export async function verifySignupOtp(input: { email: string; otp: string }) {
  const db = getMongoDb();
  await ensureUserIndexes();

  const email = input.email.trim().toLowerCase();
  const pendingSignup = await db.collection<PendingSignupDocument>("pendingSignups").findOne({ email });
  ...
  if (!otpMatches) {
    await db.collection("pendingSignups").updateOne(
      ...
      { $inc: { otpAttempts: 1 }, $set: { updatedAt: new Date().toISOString() } },
    );
...
function passwordResetEmailTemplate(otp: string) {
...
function normalizeCartItemQuantity(value: unknown) {
  const quantity = Number(value);
  if (!Number.isFinite(quantity)) return 1;
  return Math.min(Math.max(Math.trunc(quantity), 1), 99);
}

function mergeCartItems(existingItems: CartItemInput[], incomingItems: CartItemInput[]) {
...
export async function getUserCart(userId: string) {
  ...
  return {
    items: mergeCartItems([], (cart?.items ?? []) as CartItemInput[]),
  };
}

export async function saveUserCart(userId: string, items: CartItemInput[]) {
  const now = new Date().toISOString();
...
export async function mergeUserCart(userId: string, incomingItems: CartItemInput[]) {
